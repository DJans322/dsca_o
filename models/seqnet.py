...
from models.resnet_seq import build_resnet
...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        backbone, box_head = build_resnet(name="resnet50", pretrained=True) #1-4backbone  5head



        anchor_generator = AnchorGenerator(        #锚框大小和比例，可以考虑去掉小的和大的尺寸的，比例不要1的
            sizes=((32, 64, 128, 256, 512),), aspect_ratios=((0.5, 1.0, 2.0),)
        )
        head = RPNHead(
            in_channels=backbone.out_channels,            #rpn输入接收backbone输出
            num_anchors=anchor_generator.num_anchors_per_location()[0],
        )
        pre_nms_top_n = dict(    #应用nms前ROI数量
            training=cfg.MODEL.RPN.PRE_NMS_TOPN_TRAIN, testing=cfg.MODEL.RPN.PRE_NMS_TOPN_TEST
        )
        post_nms_top_n = dict( #应用nms后ROI数量
            training=cfg.MODEL.RPN.POST_NMS_TOPN_TRAIN, testing=cfg.MODEL.RPN.POST_NMS_TOPN_TEST
        )
        rpn = RegionProposalNetwork(       #生成候选框,返回box和loss
        ...
        faster_rcnn_predictor = FastRCNNPredictor(2048, 2)  #返回分类分数和锚框坐标
        reid_head = deepcopy(box_head)    #resnet5，且独立不与前面检测res5共享参数
        ...
        roi_heads = SeqRoIHeads(
            ...
            positive_fraction=cfg.MODEL.ROI_HEAD.POS_FRAC_TRAIN,   #选取的前景（正样本）占总样本的比例
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            # query          #query应该是不需要rpn操作，从reid开始进行，即网络图第二个ROI-Align开始
            boxes = [t["boxes"] for t in targets]
            box_features = self.roi_heads.box_roi_pool(features, boxes, images.image_sizes) #对roi区域进行池化操作
            box_features = self.roi_heads.reid_head(box_features)  #对池化后的特征进行re-id操作得到re-id特征
            embeddings, _ = self.roi_heads.embedding_head(box_features)  #将re-id特征转为低维特征向量(低维好算)
            return embeddings.split(1, 0)
        else:
            # gallery       #gallery从网络结构图第一个ROI-Align开始
            proposals, _ = self.rpn(images, features, targets)  #gallery时正常target为none，但是用CBGM时会有真实值
            detections, _ = self.roi_heads(         #根据网络图也可以看到，reid只需要rpn的box
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        images, targets = self.transform(images, targets)
        features = self.backbone(images.tensors)
        proposals, proposal_losses = self.rpn(images, features, targets)  #返回box和loss
