...
def visualize_target_loader(train_loader_t, epoch, output_dir):
    """
    Visualize the target domain data with pseudo bounding boxes.
    
    :param train_loader_t: DataLoader for the target domain
    :param epoch: Current epoch
    :param output_dir: Directory to save the visualized images
    """
    # Define a transform to convert tensor to image
    transform_to_image = transforms.Compose([
        transforms.Normalize(mean=[-0.485/0.229, -0.456/0.224, -0.406/0.225], std=[1/0.229, 1/0.224, 1/0.225]),
        transforms.ToPILImage()
    ])

    for batch_idx, batch in enumerate(train_loader_t):
        ...
        for idx, (image, target) in enumerate(zip(images, targets)):
            ...
            # Extract boxes and labels from the dictionary
            ...
            save_path = f"{output_dir}/epoch_{epoch}_batch_{batch_idx}_idx_{idx}.jpg"
...
def main(args):
    ...
    # pretrained_dict = torch.load("/home/guojian/paper_project/DSCA_git/exp_cuhk_seq/epoch_19.pth",map_location=device)
    # # 过滤掉不匹配的键
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        ...
        # if epoch >= target_start_epoch:
        #     visualize_target_loader(train_loader_t, epoch, output_dir="visualized_images")
